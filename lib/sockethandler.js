...
var sockethandler = function(socket){
  ...
  async.parallel({
    ...
    socket.user.faved = false;
    console.log(socket.user.favourites.forEach(console.log));
    socket.user.favourites.forEach(function(i){ console.log(i); if(socket.channel_id.equals(i))  socket.user.faved = true; })
  ...
  socket.on('channel.favourite', function(boolAdd){
    thisUser().exec(function(err, user){
      ...
      if(boolAdd){
        // we don't wan't duplicates, use addToSet
      ...
      user.save(function(err, u){
        if(err)  return socket.emit('err', err);
        socket.emit('channel.favourite', boolAdd);
      });
