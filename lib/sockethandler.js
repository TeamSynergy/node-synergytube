var debug = require('debug')('st:sockethandler');
...
var sockethandler = function(socket){
  debug('New user connected to %s... find us some profiledata', socket.request._query.channel);
  ...
  var thisUser    = function(fn){return User.findById(socket.user._id)}
  ...
  socket.channel_string = socket.request._query.channel;
  socket.channel_id = null;

  debug('User: %o', socket.user);
  ...
  async.parallel({
    ...
    users: function populateUsers(fn){
      var userids = [];
      var c = utils.socketChannelClients(socket.channel_string, io);
    ...
    guests: function countGuests(fn){
      ...
      // to the mooooon!
      debug('full: %d, guests: %d', c.length, w.length)
  ...
  socket.on('chat.more', function(userFirstMessage){
    ...
    thisChannel()
      ...
      .exec(function(err, channel){
        ...
        debug('more messages:', messages.length);
    ...
    thisChannel()
      ...
      .exec(function(err, channel){
        ...
        if(channel.playlist.length === 0){
          ...
          newPos = _.max(channel.playlist.toObject(), function(i){ return i.position; }).position + 1;
        }
        ...
        xmbd.getInfo(useritem.provider, useritem.id, function(err, info){
          ...
          channel.save(function(err){
            ...
            // if this is the channels first item, force-play it.
            if(channel.playlist.length === 1){
              io.sockets.in(socket.channel_string).emit('playlist.play', item);
            }
          });
        });
      });
  ...
  socket.on('playlist.play', function(useritem){
    ...
    thisChannel()
      ...
      .exec(function(err, channel){
        ...
        debug('force-play', useritem.name, 'from', socket.user.display_name || 'Anonymous');
  ...
  socket.on('playlist.next', function(usernext){
    if(!usernext || !usernext._id)  return socket.emit('err', 'invalid request:\n' + JSON.stringify(usernext));
    thisChannel()
      .select('playlist')
      .exec(function(err, channel){
        if(err)  return socket.emit('err', err);
        debug('item-next-claim from', socket.user.display_name || 'Anonymous');
        ...
        // are there elements in our playlist?
        ...
        if(isPlaying){
          ...
          debug('change server-current to:', next.name);

          if(!next._id.equals(usernext._id) && !config.synergy.force_sync){
            debug('user\'s next is not the server\'s next item. synced');
            socket.emit('playlist.play', next);
          }

          if(config.synergy.force_sync){
            debug('force all users to sync to the newest item');
      ...
      .exec(function(err, channel){
        ...
        for (var i = 0; i < channel.playlist.length; i++) {
          if(channel.playlist[i]._id.equals(useritem._id)){
            ...
            debug('remove item:', useritem._id);
        ...
        if(found){
          ...
          debug('item to remove not found');
        }
   ...
   * Misc methods
   */
  socket.on('channel.favourite', function(boolAdd){
    thisUser().exec(function(err, user){
      if(err)  return socket.emit('err', err);

      if(boolAdd){
        // we don't want duplicates, use addToSet
        user.favourites.addToSet(socket.channel_id);
      } else {
        // just remove it then
        user.favourites.pull(socket.channel_id);
      }

      // save the user
      user.save(function(err, u){
        if(err)  return socket.emit('err', err);
        utils.socketEmitToUser(socket.channel_string, socket.user._id, io, 'channel.favourite', boolAdd);
      });
